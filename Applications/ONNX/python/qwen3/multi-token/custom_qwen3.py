...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.register_buffer(
            "scaling_factor", torch.tensor([self.scaling], dtype=torch.float32)
        )
    ...
    def forward(
        ...
        query_states = self.q_norm(
            self.q_proj(hidden_states).view(q_hidden_shape), eps
        ).transpose(1, 2)
        key_states = self.k_norm(
            self.k_proj(hidden_states).view(kv_hidden_shape), eps
        ).transpose(1, 2)
        ...
        attn_weights = (
            torch.matmul(query_states, key_states.transpose(2, 3)) * self.scaling_factor
        )
        ...
        attn_output = attn_output.reshape(
            1, 1, slen, self.config.hidden_size
        ).contiguous()
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            hidden_states = decoder_layer(
                hidden_states.reshape(
                    1, 1, self.config.max_position_embeddings, self.config.hidden_size
                ),
                cos,
                sin,
                eps,
